...
import { TASK_PRIORITY_LABELS, TASK_PRIORITY_COLORS, type TaskPriority } from '@/constant/task';
...
export const columns: ColumnDef<Task, unknown>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Button variant="ghost" size="icon-sm" onClick={row.getToggleExpandedHandler()}>
          <ChevronRightIcon className={cn('size-4 transition-transform', row.getIsExpanded() && 'rotate-90')} />
        </Button>
      );
    },
    size: 32,
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge variant="secondary" className={cn('text-xs', TASK_PRIORITY_COLORS[priority])}>
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-1 justify-end">
        ...
        <DeleteTaskDialog taskId={row.original.id} taskTitle={row.original.title} />
      </div>
    ),
  },
];
